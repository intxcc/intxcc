...
const StoryText = (
  <p>
    ...
    <br />
    The next big step was when the problem araised, that on small displays, as well as - of course - in Internet Explorere, verything would break apart. So I decided after thinking for a long time, to create the complete site, from ground up, in a responsive way, again. As fallback, for small displays and Internet Explorer. I thought about responsiveness from the get-go, but the design with the fading triangles would not have made any sense on small screens, so the only step, without having to compromise on the desktop layout I am very happy with, was to create two different views but a uniform state.<br />
    <br />
    That step was great in hindsight. I am still able to reuse some components - like the skill map - that makes sense in the fallback version and could fully optimize each view version for its own purpose without having to make any compromises. And the state was no problem at all, because the state is the same, no matter which view one does use. Only some view specific states are only used in one version, but they could - if one would need to - be used in the other view at any time.<br />
    <br />
    ...
    <br />
    Yeah. Alright .. ok. There stood: <i>To demonstrate my skills.</i> But <i>To make a great website.</i> just has a better ring to it, don&apos;t you think?
  </p>
)
